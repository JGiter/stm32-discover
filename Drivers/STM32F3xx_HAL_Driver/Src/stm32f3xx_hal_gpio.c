/**
  ...
  ******************************************************************************
  ...
  push-pull type and the IO speed can be selected depending on the VDD value.
    ...
    (+) The microcontroller IO pins are connected to onboard
  peripherals/modules through a multiplexer that allows only one peripheral
  alternate function (AF) connected to an IO pin at a time. In this way, there
  can be no conflict between peripherals sharing the same IO pin.

    (+) All ports have external interrupt/event capability. To use external
  interrupt lines, the port must be configured in input mode. All available
  GPIO pins are connected to the 16 external interrupt/event lines from EXTI0
  to EXTI15.
  ...
  (interrupt or event) and the corresponding trigger event (rising or falling
  or both). Each line can also be masked independently.
  ...
  is configured through "Speed" member from GPIO_InitTypeDef structure.
        (++) In alternate mode is selection, the alternate function connected
  to the IO is configured through "Alternate" member from GPIO_InitTypeDef
  structure.
  ...
  purpose (PC14 and PC15U, respectively) when the LSE oscillator is off. The
 ...
 * values.
 * @param  GPIOx where x can be (A..F) to select the GPIO peripheral for
 * STM32F30X device or STM32F37X device
 ...
 *
 * @note   This function uses GPIOx_BSRR and GPIOx_BRR registers to allow
 * atomic read/modify accesses. In this way, there is no risk of an IRQ
