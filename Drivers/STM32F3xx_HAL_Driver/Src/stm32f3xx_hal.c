...
#define __STM32F3xx_HAL_VERSION                                               \
  ((__STM32F3xx_HAL_VERSION_MAIN << 24U)                                      \
   ...
   | (__STM32F3xx_HAL_VERSION_SUB2 << 8U) | (__STM32F3xx_HAL_VERSION_RC))
...
/** @defgroup HAL_Exported_Functions_Group1 Initialization and
...
@verbatim
 ...
 ===============================================================================
    [..]  This section provides functions allowing to:
      (+) Initializes the Flash interface, the NVIC allocation and initial
clock configuration. It initializes the systick also when timeout is needed and
the backup domain when enabled.
 ...
 * @note   This function is called at the beginning of program after reset and
 ...
 * Priority group 4
 *
 * @note   The time base configuration is based on MSI clock when exiting from
 * Reset. Once done, time base tick start incrementing. In the default
 * implementation,Systick is used as source of time base. The tick variable is
 * incremented each 1ms in its ISR.
 * @retval HAL status
 */
HAL_StatusTypeDef
HAL_Init (void)
 ...
 * uwTick is incremented. The function is declared as __Weak  to be overwritten
 ...
 * Tick increment is resumed. The function is declared as __Weak  to be
